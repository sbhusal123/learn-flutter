# Some Useful Widgets:

[Widget Catalog](https://docs.flutter.dev/ui/widgets/material)


## 1. Image Widget:


**Image From URL:**

```dart
Image(
    image: NetworkImage('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRF98sbQeNeSMz3nbGKwwnh4XyFU-ojyeNghA&s')
)
```

**Image From Asset:**

```dart
Image(
    image: AssetImage('assets/image.png')
)
```

![Image Widget](./images/image_widget.png)

To use a image from assets, image must be first defined in `pubspec.yaml` file.

```yml
assets:
  - assets/image.png
```

If there are multiple images:

```yml
assets:
  - assets/space-1.jpg
  - assets/space-2.jpg
  - assets/space-3.jpg
```

Or simply:

```yml
assets:
  - assets
```

## Icon Widget:

```dart
Icon(
    Icons.star, // use existing icons provided by flutter
    color: Colors.yellow,
    size: 50,
)
```

![Icon Widget](./images/icon_widget.png)

## Icon Button:

```dart
IconButton(onPressed: () => print("Pressed"), icon: Icon(Icons.home))
```

![Icon Button](./images/icon_button.png)


## Divider Widget:

Divider is used to draw a horizontal line between widgets.

```dart
Divider(color: Colors.grey[600], height: 60)
```

![Divider](./images/divider.png)


## Center Widget:

Center is used to center a widget.

```dart
Center(
    child: Text('Hello World'),
)
```

![Center](./images/center.png)

## CircleAvatar Widget:

CircleAvatar is used to display a circle avatar.

```dart
CircleAvatar(
    backgroundImage: AssetImage('assets/images/chun-li.jpg'),
    radius: 50,
)
```

![CircleAvatar](./images/circle_avatar.png)

## ListView Widget:

```dart
ListView(
    children: [
      Text('Hello World'),
      Text('Hello World'),
      Text('Hello World'),
    ],
)
```

![ListView](./images/listview.png)

## ListView.builder Widget:

```dart
ListView.builder(
    itemCount: 10,
    itemBuilder: (context, index) {
      return Text('Hello World');
    },
)
```

![ListView.builder](./images/listview_builder.png)

## Listtitle Widget:

```dart
ListTile(
    leading: Icon(Icons.star),
    title: Text('Hello World'),
    subtitle: Text('Hello World'),
    trailing: Icon(Icons.star),
)
```

![ListTile](./images/listtile.png)

Here, leading, title, subtitle and trailing are widget themselves.



## Visibility Widget:

- Use to controll the visibility with `visible` boolean parameter.

```dart
Visibility(
    visible: <true/false>, // boolean parameter
    child: Padding(
    padding: EdgeInsetsGeometry.all(10),
    child: Container(
        width: 100,
        height: 100,
        color: Colors.indigoAccent,
        child: Center(child: Text('Visibility Container'),),
    ),
    ),
);
```

## SnackBar Widget:

![SnackBar](./images/SnackBar.png)

```dart
ElevatedButton(
  onPressed: () {
    final snackBar = SnackBar(
      content: const Text('Warning: Connection lost!'),
      backgroundColor: Colors.redAccent, // Custom background color
      duration: const Duration(seconds: 2), // Custom display time
      behavior: SnackBarBehavior.floating, // Makes it float above the bottom edge
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(10), // Rounded corners
      ),
      margin: const EdgeInsets.all(16), // Margin for floating behavior
    );

    ScaffoldMessenger.of(context).showSnackBar(snackBar);
  },
  child: const Text('Show SnackBar'),
)
```

Here we display a `snackbar` on a press of a button.
